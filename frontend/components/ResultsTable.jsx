            ...
            <div className={styles.listWrapper}>
                {sortedResults.map((result, index) => {
                    ...
                    return (
                        <div
                            key={result.id}
                            className={`${styles.candidateCard} ${activeCandidateId === result.id ? styles.activeCard : ""}`}
                            onClick={() => onViewCandidate(result.id === activeCandidateId ? null : result.id)}
                        >
                            <div className={styles.cardHeader}>
                                <h4 className={styles.fileName}>{result.name}</h4>
                            </div>
                            ...
                            <div className={styles.cardFooter}>
                                <div className={`${styles.scoreBadge} ${scoreClass}`}>
                                    <span className={styles.scoreValue}>{score}% Match</span>
                                </div>
